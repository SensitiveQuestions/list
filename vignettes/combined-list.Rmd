---
title: "Combining List Experiments with Direct Questions using `combined.listDirect()`"
...
```{r}
library(list) # this will need to change when incorporated
...
The `combined.list()` function automatically conducts two placebo tests that can check the assumptions underlying the list experimental design.
...
out.2 <- combined.listDirect(formula = Y ~ Z, data = df, treat = "Z", direct = "D")
...
out.3 <- combined.listDirect(formula = Y ~ Z, data = df, treat = "Z", direct = "D")
...
out.4 <- combined.listDirect(formula = Y ~ Z, data = df, treat = "Z", direct = "D")
out.4

# Conduct estimation with covariate adjustment
# Just add the covariate on the right-hand side of the formula
out.5 <- combined.listDirect(formula = Y ~ Z + X, data = df, treat = "Z", direct = "D")
out.5
```
